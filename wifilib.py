...
def read_bf_file1(filename, decoder="python") -> object:
    with open(filename, "rb") as f:
        bfee_list = []
        field_len = int.from_bytes(f.read(2), byteorder='big', signed=False)
    ...
    len_arr = len(bfee_list[0])
    i = 0
    # print("len(bfee_list):",len(bfee_list))
    for array in bfee_list:
        i = i + 1
        #print("总：",i)
        # print("*******************",len(array))
        if len(array) != 393:
            diu11.append(len(array))
            diu1.append(i)
            continue
        # % Read size and code
    ...
    print("长度不为393：  ", diu1)
    print("长度不为393位：", diu11)
    print("长度不为393数：", len(diu11))
    print("长度为393，但不是CSI：", diu2)
    return dicts
...
def get_scale_csi(csi_st):
    ...
    # print("csi_st", csi_st)
    # print("get_total_rss(csi_st):", get_total_rss(csi_st))
